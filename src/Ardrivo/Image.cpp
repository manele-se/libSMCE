...
Image::Image() : Image(ENCODING_NONE, (const uint8_t*)NULL, 0, 0) {}

Image::Image(int encoding, const uint8_t* data, int width, int height)
    : _encoding(encoding), _data(data), _width(width), _height(height) {}
...
int Image::encoding() const { return _encoding; }

const uint8_t* Image::data() const { return _data; }

int Image::width() const { return _width; }

int Image::height() const { return _height; }

Image::operator bool() const { return (_encoding != ENCODING_NONE && _data != NULL && _width > 0 && _height > 0); }
