...
void ArduinoGraphics::background(uint32_t color) { background(COLOR_R(color), COLOR_G(color), COLOR_B(color)); }
...
void ArduinoGraphics::set(int x, int y, uint32_t color) { set(x, y, COLOR_R(color), COLOR_G(color), COLOR_B(color)); }

size_t ArduinoGraphics::write(uint8_t b) {
    if (b != 0xc2 && b != 0xc3) {
        _textBuffer = _textBuffer + String((char)b);
    }
